...
use simplelog::{
    CombinedLogger, ConfigBuilder, LevelFilter, SharedLogger, TermLogger, WriteLogger,
};
    ...
    for sink in LOG_SINKS {
        ...
        sinks.push(match env_var.as_ref() {
            ...
            file => {
                ...
                let fd = Reopen::new(Box::new(move || open_log_file(file_name.to_owned()))).expect(
                    &format!("Unable to open log file for {}: {}", &env_var_name, file),
                );
                // If system is unix, watch for SIGHUP to reopen the file (logrotate)
                #[cfg(unix)]
                actix_web::rt::spawn(watch_sighup(fd.handle()));
...
#[cfg(unix)]
async fn watch_sighup(handle: Handle) -> std::io::Result<()> {
